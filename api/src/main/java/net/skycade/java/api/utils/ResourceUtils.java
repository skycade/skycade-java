package net.skycade.java.api.utils;
...
public final class ResourceUtils {
   ...
   * @param clazz  the class to use for extracting the resource
   * @throws URISyntaxException if the URI is invalid
   * @throws IOException        if an I/O error occurs
   */
  public static void extractResource(String source, Class<?> clazz)
      throws URISyntaxException, IOException {
    //  final URI uri = Objects.requireNonNull(ResourceUtils.class.getResource("/" + source)).toURI();
    final URI uri = Objects.requireNonNull(clazz.getResource("/" + source)).toURI();
    FileSystem fileSystem = null;

    // log the resource being extracted
    SkycadeJavaBootstrapper.LOGGER.info("Extracting resource: " + source);
